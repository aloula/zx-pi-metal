...
#define ZX128_TSTATES_PER_FRAME 70908  /* 128K/+2 total T-states per frame */
#define ZX128_TSTATES_PER_LINE  228    /* 128K/+2 T-states per scanline */
#define ZX128_LINES_PER_FRAME   311    /* 128K/+2 total scanlines per frame */
#define ZX_DISPLAY_LINES       192     /* Active display lines */
#define ZX_FIRST_DISPLAY_LINE  64      /* First scanline with pixel data */
#define ZX128_FIRST_DISPLAY_LINE 63    /* 128K/+2 first display scanline */
...
#define ZX128_FIRST_CONTENDED  14361
...
#define ZX_MAX_BEEPER_EVENTS   8192
#define ZX_MAX_AY_EVENTS       4096
...
/* ===================================================================
 * AUDIO EVENTS (for mid-frame updates)
 * =================================================================== */
...
typedef struct {
    uint32_t tstates;  /* Frame T-state when the register was written */
    uint8_t  reg;      /* AY register index (0-15) */
...
typedef struct ZXSpectrum {
    ...
    uint8_t ay_registers_start[ZX_AY_REG_COUNT]; /* AY registers at start of frame */
    ...
    /* Audio: event logs for the current frame. */
    ZXBeeperEvent beeper_events[ZX_MAX_BEEPER_EVENTS];
    int beeper_event_count;
    ZXAYEvent ay_events[ZX_MAX_AY_EVENTS];
    int ay_event_count;
    uint32_t ay_tone_counter[3];
    uint8_t ay_tone_output[3];
    uint32_t ay_noise_counter;
    uint32_t ay_noise_lfsr;
    uint32_t ay_envelope_counter;
    uint8_t ay_noise_output;
    uint8_t ay_envelope_step;
    uint8_t ay_envelope_attack;
    uint8_t ay_envelope_hold;
    uint8_t ay_envelope_alternate;
    uint8_t ay_envelope_holding;
...
/* Set separate 128K ROM banks (16KB each). Either pointer may be NULL,
 * in which case ROM 0 is used as a fallback. */
