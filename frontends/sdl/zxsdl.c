...
static int load_rom_file(const char *path, uint8_t *dst) {
    FILE *f = fopen(path, "rb");
    if (!f) return 0;

    size_t got = fread(dst, 1, 16384, f);
    int extra = fgetc(f);
    fclose(f);

    return got == 16384 && extra == EOF;
}

static void apply_optional_128k_roms(ZXSpectrum *zx,
                                     uint8_t *rom0, int have_rom0,
                                     uint8_t *rom1, int have_rom1) {
    if (have_rom0)
        zx_set_128k_roms(zx, rom0, have_rom1 ? rom1 : NULL);
}
...
int main(int argc, char **argv) {
    ...
    uint8_t *rom128_0 = (uint8_t *)malloc(16384);
    uint8_t *rom128_1 = (uint8_t *)malloc(16384);
    int have_rom128_0 = 0;
    int have_rom128_1 = 0;
    zx_init(zx, zx_spectrum_rom);

    if (rom128_0 && rom128_1) {
        have_rom128_0 = load_rom_file("roms/128-0.rom", rom128_0);
        have_rom128_1 = load_rom_file("roms/128-1.rom", rom128_1);
        apply_optional_128k_roms(zx, rom128_0, have_rom128_0, rom128_1, have_rom128_1);
    }
    ...
    while (running) {
        ...
        if (tzx_is_playing(&tape)) {
            tzx_set_machine(&tape, zx->machine_128k);
            do {
