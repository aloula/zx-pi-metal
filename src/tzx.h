...
typedef struct {
    ...
    /* Current machine mode for TZX conditional blocks. */
    uint8_t machine_128k;
} TZXPlayer;
...
/* Set the current machine mode for conditional TZX blocks.
 * Pass 0 for 48K mode, non-zero for 128K-family mode. */
void tzx_set_machine(TZXPlayer *p, int machine_128k);
